...
const CHART_COLORS = ['#2563EB', '#3B82F6', '#06B6D4', '#6366F1', '#16A34A', '#D97706', '#EC4899', '#8B5CF6'];

// Heatmap color scale - blue to red for correlations
const HEATMAP_COLORS = ['#1E3A8A', '#2563EB', '#3B82F6', '#38BDF8', '#FDE047', '#F97316', '#DC2626', '#991B1B'];
      ...
      <div className="h-[320px] flex items-center justify-center">
        <svg width="100%" height="100%" viewBox="0 0 500 320" preserveAspectRatio="xMidYMid meet">
          ...
          <g transform="translate(60, 280)">
            ...
            <text x="0" y="-4" fontSize="9" fill="#94A3B8">-1.0</text>
            <text x="200" y="-4" fontSize="9" fill="#94A3B8" textAnchor="end">1.0</text>
          </g>
        </svg>
      </div>
    </div>
...
export default function ChartViewer({ chartUrl, tableData, chartData, title = 'Generated Visualization', message }) {
  ...
  const handleExportPdf = async () => {
    if (!message) {
      toast.error('Analysis data not available for PDF export');
      return;
    }

    try {
      const datasetName = message.datasetId || message.dataset_id || 'analysis';
      const blob = await api.exportPdf(message, datasetName);
  ...
  if (!fullChartUrl && !rechartsData) {
    return (
      <div className="p-8 text-center card text-text-muted text-xs md:text-sm">
        <BarChart3 className="w-8 h-8 text-border mx-auto mb-2" aria-hidden="true" />
          ...
          <ResponsiveContainer width="100%" height="100%">
            {chartType === 'line' ? (
              <LineChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 40 }}>
                ...
                <XAxis dataKey="name" stroke="#94A3B8" fontSize={11} angle={-20} textAnchor="end" />
                <YAxis stroke="#94A3B8" fontSize={11} />
                <Tooltip contentStyle={tooltipStyle} />
                <Legend />
                <Line type="monotone" dataKey="value" stroke="#2563EB" strokeWidth={2.5} dot={{ r: 4, fill: '#2563EB' }} name={config.yAxisLabel || 'Value'} />
              ...
              <AreaChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 40 }}>
                ...
                <YAxis stroke="#94A3B8" fontSize={11} />
                <Tooltip contentStyle={tooltipStyle} />
                <Area type="monotone" dataKey="value" stroke="#2563EB" fill="#2563EB" fillOpacity={0.25} strokeWidth={2} />
              </AreaChart>
            ) : chartType === 'pie' ? (
              <PieChart>
                <Tooltip contentStyle={tooltipStyle} />
                <Legend />
                <Pie data={data} dataKey="value" nameKey="name" innerRadius={40} outerRadius={120} fill="#2563EB" label>
              ...
              <ScatterChart margin={{ top: 20, right: 30, left: 20, bottom: 40 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
                <XAxis dataKey="x" stroke="#94A3B8" fontSize={11} name={config.xAxisLabel || 'X'} />
                <YAxis dataKey="y" stroke="#94A3B8" fontSize={11} name={config.yAxisLabel || 'Y'} />
                <Tooltip contentStyle={tooltipStyle} />
              ...
              <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 40 }}>
                ...
                <Tooltip contentStyle={tooltipStyle} />
                <Legend />
                <Bar dataKey="value" fill="#2563EB" radius={[4, 4, 0, 0]} name={config.yAxisLabel || 'Value'} />
                {seriesKeys.slice(1).map((key, idx) => (
  ...
  return (
    <div className={`card overflow-hidden shadow-sm ${isFullscreen ? 'fixed inset-4 z-50 shadow-2xl flex flex-col bg-white/95 backdrop-blur-2xl' : ''}`}>
      {/* Chart Header Toolbar */}
      <div className="p-3.5 border-b border-border bg-surface-secondary flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          ...
          {fullChartUrl && rechartsData && (
            <div className="flex items-center bg-surface border border-border rounded-lg overflow-hidden mr-1">
              <button
                type="button"
                onClick={() => setViewMode('static')}
                className={`px-2.5 py-1 text-xs font-medium transition-colors ${viewMode === 'static' ? 'bg-primary text-white' : 'text-text-muted hover:text-text-primary'}`}
              >
                Static
              </button>
              <button
                ...
                className={`px-2.5 py-1 text-xs font-medium transition-colors border-x border-border ${viewMode === 'interactive' ? 'bg-primary text-white' : 'text-text-muted hover:text-text-primary'}`}
              >
                Interactive
              </button>
              <button
                ...
                className={`px-2.5 py-1 text-xs font-medium transition-colors ${viewMode === 'both' ? 'bg-primary text-white' : 'text-text-muted hover:text-text-primary'}`}
              >
                Both
              </button>
            </div>
      ...
      {/* Chart Body */}
      <div className={`p-4 bg-background ${isFullscreen ? 'flex-1 overflow-auto' : ''}`}>
          ...
          {showStatic && (
            <div className="flex items-center justify-center min-h-[280px]">
              <img
                ...
                className="max-h-[420px] w-auto object-contain rounded-xl border border-border shadow-sm"
              />
            </div>
          )}
          {showInteractive && (

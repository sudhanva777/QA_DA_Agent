...
export default function HowItWorks() {
  ...
  return (
    <div ref={containerRef} className="relative py-32 lg:py-48 px-4 sm:px-6 lg:px-8">
      <div className="absolute inset-0 -z-10 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-[var(--accent-primary)]/3 rounded-full blur-[150px]" />
        <div className="absolute top-1/4 right-1/4 w-[400px] h-[400px] bg-[var(--accent-secondary)]/3 rounded-full blur-[120px]" />
        <div className="absolute bottom-1/4 left-1/4 w-[400px] h-[400px] bg-[var(--accent-tertiary)]/3 rounded-full blur-[120px]" />
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative z-10">
          <div className="hidden lg:block absolute left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-[var(--accent-primary)]/30 to-transparent -translate-x-1/2" />
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-full lg:w-1/2 relative ${isLeft ? 'pr-8 text-right' : 'pl-8 text-left'}`}>
                    <div className={`card-elevated relative p-6 sm:p-8 ${isVisible ? 'border-[var(--accent-primary)]/30 shadow-[0_0_30px_rgba(139,92,246,0.15)]' : ''} transition-all duration-500 animate-fade-in-up`}>
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-3xl font-black text-[var(--accent-primary)] font-mono-tight">{step.number}</span>
                      ...
                      {/* Feature highlights */}
                      <div className="mt-6 pt-4 border-t border-[var(--border-subtle)] grid grid-cols-1 sm:grid-cols-2 gap-3 text-left">
                        {step.features.map((feat, i) => (
                          <div key={i} className="flex items-center space-x-2 text-xs text-[var(--text-primary)]">
                            <CheckCircle2 className="w-3.5 h-3.5 text-[var(--accent-tertiary)] flex-shrink-0" />
                            <span>{feat}</span>
                          </div>
                        ))}
                      </div>
          ...
          {PIPELINE_STEPS.map((step, index) => {
            ...
            return (
              <div
                key={step.number}
                ref={(el) => { if (el) el.dataset.index = index; }}
                className={`card-elevated p-6 transition-all duration-500 ${isVisible ? 'border-[var(--accent-primary)]/30 shadow-[0_0_30px_rgba(139,92,246,0.15)]' : 'opacity-50'} animate-fade-in-up`}
              >
                <div className="flex items-center space-x-3 mb-4">
                  <span className="text-2xl font-black text-[var(--accent-primary)] font-mono-tight">{step.number}</span>
